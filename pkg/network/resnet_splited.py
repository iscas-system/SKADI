...
# 残差连接：ResNet 中提出并使用了残差连接， 这个想法是将层的输入与层的输出相加，输出 = 层（输入）+ 输入
...
# 捷径 Shortcut：有时候残差没有相同的输出维度，所以无法将它们相加。
# 所以就需要使用conv(带+的黑色箭头)来投影输入，以匹配输出的特性

# ref：https://segmentfault.com/a/1190000041821627?sort=votes
# 瓶颈块 BottleNeck：BottleNeck 块接受大小为 BxCxHxW 的输入
# 它首先使用1x1 卷积将其缩减为 BxC/rxHxW
# 然后再应用 3x3 卷积，最后再使用 1x1 卷积将输出重新映射到与输入相同的特征维度BxCxHxW 。
# 这比使用三个 3x3 转换要快的多，由于中间层减少输入维度，所以将其称之为“BottleNeck”。
...
class ResNet(nn.Module):
    def __init__(
        ...
        for layer in self.moduleList:
            ...
            if isinstance(layer, avgpool_fc):
                maps["AdaptiveAvgPool2d"] += 1
                maps["Linear"] += 1
            else:
                # print(len(layer))
                for seq in layer:
                    if isinstance(seq, Bottleneck):
                        maps["Conv2d"] += 3
                        maps["BatchNorm2d"] += 3
                        maps["ReLU"] += 1
                        if seq.downsample != None:
                            maps["Conv2d"] += 1
                            maps["BatchNorm2d"] += 1
                    elif isinstance(seq, BasicBlock):
                        maps["Conv2d"] += 2
                        maps["BatchNorm2d"] += 2
                        maps["ReLU"] += 1
                        if seq.downsample != None:
...
# https://blog.csdn.net/Haoyee1/article/details/124740736
...
if __name__ == '__main__':
    ...
    exec_times = np.array([])
    for i in range(100):
        input = torch.rand(1, 3, 224, 224).half().cuda()
        res = model(input)
    for i in range(100):
        init_input = torch.rand(32, 3, 224, 224)
        start = time.time()
        input = init_input.half().cuda()
        res = model(input)
