...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0:
            ...
            _H, _W = _make_divisible(
                H, self.window_size), _make_divisible(W, self.window_size),
    ...
    def forward(self, x):
        ...
        _H, _W = _make_divisible(
            H, self.window_size), _make_divisible(W, self.window_size),
        x = x.permute(0, 3, 1, 2)
        x = F.interpolate(x, [_H, _W], mode='bicubic',
        ...
        if type(self.attn_mask) != type(None):
            attn_mask = torch.tensor(self.attn_mask).cuda(
            ) if x.is_cuda else torch.tensor(self.attn_mask)
        else:
            attn_mask = None
        # nW*B, window_size*window_size, C
        ...
        shifted_x = window_reverse(
            attn_windows, self.window_size, _H, _W)  # B H' W' C
