...
def inception_v3(pretrained=False, progress=False, **kwargs):
    ...
    #         del model.AuxLogits
    #     # print("inception_v3", model.moduleList)
    #     return model

    model = Inception3(**kwargs)
    # if pretrained:
    file_name = model_urls["inception_v3_google"].split("/")[-1]
    ...
    if os.path.exists(model_path):
        ...
        state_dict = load_state_dict_from_url(
            model_urls["inception_v3_google"], progress=progress, check_hash=True, model_dir=model_dir)
    model.load_state_dict(state_dict)
    return model
...
class Inception3(nn.Module):
    def __init__(
        ...
        # print("inception_v3", self.modelList)
        ...
        for layer in self.modelList:
            ...
            if isinstance(layer, inception_avgpool_dropout_flatten_fc):
                maps["AdaptiveAvgPool2d"] += 1
                maps["Linear"] += 1
                maps["Dropout"] += 1
            else:
                for seq in layer:
                    if isinstance(seq, BasicConv2d):
                        maps["BatchNorm2d"] += 1
                        maps["Conv2d"] += 1
                    elif isinstance(seq, InceptionA):
                        maps["BatchNorm2d"] += 7
                        maps["Conv2d"] += 7
                    elif isinstance(seq, InceptionB):
                        maps["BatchNorm2d"] += 4
                        maps["Conv2d"] += 4
                    elif isinstance(seq, InceptionC):
                        maps["BatchNorm2d"] += 10
                        maps["Conv2d"] += 10
                    elif isinstance(seq, InceptionD):
                        maps["BatchNorm2d"] += 6
                        maps["Conv2d"] += 6
                    elif isinstance(seq, InceptionE):
                        maps["BatchNorm2d"] += 9
                        maps["Conv2d"] += 9
    ...
    def generate_model_list(self):
        ...
        list.append(
            nn.Sequential(self.Conv2d_3b_1x1,
                          self.Conv2d_4a_3x3, self.maxpool2)
        )
        ...
        list.append(
            inception_avgpool_dropout_flatten_fc(
