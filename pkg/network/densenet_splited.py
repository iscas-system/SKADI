...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__(
        ...
        moduleList: List[nn.Module] = None,
    ) -> None:
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.moduleList.append(nn.Sequential(conv0, norm0, relu0, pool0))
        ...
        for i, num_layers in enumerate(block_config):
            ...
            self.moduleList.append(block)
            num_features = num_features + num_layers * growth_rate
            if i != len(block_config) - 1:
                trans = _Transition(
                    num_input_features=num_features, num_output_features=num_features // 2)
                self.features.add_module("transition%d" % (i + 1), trans)
                self.moduleList.append(trans)
        ...
        self.moduleList.append(new_classifier(self.classifier))
        ...
        for layer in self.moduleList:
            ...
            if isinstance(layer, new_classifier):
                maps["ReLU"] += 1
                maps["AdaptiveAvgPool2d"] += 1
                maps["Linear"] += 1
            elif isinstance(layer, _DenseBlock):
                num = layer.num_layers*2
                maps["Conv2d"] += num
                maps["ReLU"] += num
                maps["BatchNorm2d"] += num
                ...
                for seq in layer:
                    if isinstance(seq, nn.Conv2d):
                        ...
                        maps["MaxPool2d"] += 1
                    elif isinstance(seq, nn.AvgPool2d):
                        maps["AdaptiveAvgPool2d"] += 1
                    else:
                        print("seq unimplement ", type(seq))
            else:
                print("layer unimplement ", type(layer))
...
def _load_state_dict(model: nn.Module, state_dict, progress: bool) -> None:
...
def _densenet(
    ...
    **kwargs: Any,
    ...
    model = DenseNet(growth_rate, block_config,
                     num_init_features, pretrained, progress, **kwargs)
    if pretrained:
        ...
        _load_state_dict(model, state_dict, progress)
        # model.load_state_dict(state_dict)

    return model


def densenet121(*, pretrained: bool = True, progress: bool = False, **kwargs: Any) -> DenseNet:
...
def densenet161(*, pretrained: bool = True,  progress: bool = False, **kwargs: Any) -> DenseNet:
...
def densenet169(*, pretrained: bool = True, progress: bool = False, **kwargs: Any) -> DenseNet:
...
def densenet201(*, pretrained: bool = True, progress: bool = False, **kwargs: Any) -> DenseNet:
    ...
    return _densenet("densenet201", 32, (6, 12, 48, 32), 64, progress, pretrained, **kwargs)
...
if __name__ == '__main__':
    ...
    # input = torch.rand(8, 3, 224, 224).half().cuda()
    # print(model(input))

    model_func = densenet161
    model = model_func().half().cuda().eval()
    ...
    submodel1 = nn.Sequential(*_submodules)
